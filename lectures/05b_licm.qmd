...
d = 5 + b is loop invariant of b
```

In ssa a variable is loop invariant if it does not have a phi function at the loop of the 
loop, or its a phi function and its inputs come from outside of the loop
...
## while loop may not execute sometimes need extra basic blocks 
...
we could also guess that the loop will run more then once 

## when is ok to move a computation 

1. no side effects - cannot move  alloc 10;
