...
traces through a program function given some inputs and records the pytorch operations that executed 
...
at a call python builds a frame: 

(codeObject, list of arguments)

and calls a function evalFrame(frame)

Torch.dynamo changes this to:

1. uses the codeObject as a key into a hash table 
1. hash table has guards (lookup fails if guards fail)
1. if lookup fails
   a. analysis of bytecode
   a. generate a static graph (torch parts)
   a. run the compiler to generate a function
   a. generate a new codeObject (non-torch parts) calls function
   a. add to hash
1. evalFrame using the found codeObject
...
walk the bytecodes of the function up to control flow 
...
1. calls guards - maybe reform graph and compile
1. gets multiple graphs + functions 
1. some functions may call the graph processor
1. all control flow stays in the functions 
