...
Why is it hard to learn about gpus?

1. architectural variations - all the vendors keep changing the architecture. Different vendors build different hardware
Graphics programs run on all vendors, because vendors ship the ir and do finalization on the device 
1. inconsistent terminology - vendors use different terms for the same features and reuse cpu terms somewhat differently for instance a "Cuda core" is not the same as a 'cpu core'
1. Legacy terminology - some terms come from gpu history of graphics for example a shader is a program
1. software abstractions - big frameworks hide the details 
1. CUDA is the market leader, so most explanations use CUDA terms. For instance AMD uses 'wave' to mean  more or less the same as a CUDA 'warp' but lots of AMD documentation uses 'warps'
1. CUDA is both a programming language and a programming model, so you can have CUDA Fortran, CUDA python etc. 
...
both vendors use the CUDA programming model  
