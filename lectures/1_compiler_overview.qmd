...
In 1952, she had an operational link-loader, which she called a compiler. She later said, "Nobody believed that," and that she "had a running compiler and nobody would touch it. They told me computers could only do arithmetic."

[![hopper](images/Grace_Hopper_and_UNIVAC.jpg)]
...
In gcc the IR is somewhat C based, for instance there are pointers but there is no way to talk about garbage collection without hacks.

in about 2006 LLVM (low level virtual machine) appeared. This changed the model to look like a library. 
...
```{mermaid}
graph TB;
A[TEXT_Version of BRIL]--> B0[BRIL in JSON] --> B1[ASSIGNMENT] --> B2[BRIL interpreter];
```
...
## Compiler Assumptions (Let's talk about how many are still true)
...
1) DSL (Doman specific languages for AI)
